...
# BYPASS: Ask for the key directly in the app interface
api_key_input = st.text_input("Enter your Gemini API Key (starts with AIza...):", type="password")

if not api_key_input:
    st.warning("Please enter your API key above to unlock the file uploader.")
else:
    # Configure the AI with the key typed into the box
    try:
        genai.configure(api_key=api_key_input)
        # Using 1.5-flash as it is faster and highly reliable for strict data extraction
    ...
    if uploaded_file is not None:
        with st.spinner("AI is reading the invoice and calculating TDS..."):
            try:
                ...
                st.error(f"Extraction failed: Ensure the key is valid and the file is a digital PDF. Error details: {e}")
